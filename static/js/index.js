...
var CodeHighlight = require('./code_highlight');
var Search = require('./search');

var RTD = function () {
    function RTD() {
        _classCallCheck(this, RTD);
        ...
        if (isApi) {
            var longnameSelector = doc.longname.replace(/[^A-Za-z0-9]/g, '_');
            this.$.selectedApiSubItem = $('#' + longnameSelector + '_sub');
            this.$.selectedApiSubItem.removeClass('hidden');
            var selectedApiItem = this.$.selectedApiSubItem.prev();
            selectedApiItem.addClass('selected');
            var selectedApiItemParent = this.$.selectedApiSubItem.parent();
            selectedApiItemParent.removeClass('hidden');
            // Try to position selectedApiItem at the top of the scroll container.
            var navScrollTop = this.$.scroll.get(0).getBoundingClientRect().top;
            var navItemTop = selectedApiItem.get(0).getBoundingClientRect().top;
    ...
    _createClass(RTD, [{
        ...
        value: function events() {
            ...
            this.$.nav.find('.nav-api').each(function () {
                $(this).find('.toggle-subnav').filter(function () {
        ...
        key: 'selectHref',
        value: function selectHref() {
            $('.sub-nav-item a').removeClass('selected');
            var item = document.querySelector('a[href$="' + location.pathname.substr(1) + location.hash + '"]');
    ...
    _createClass(Search, [{
        ...
        value: function events() {
            ...
            $(window).on('click', function (event) {
                if (!_this.$.searchContainer[0].contains(event.target)) {
                    _this.clear();
                }
            });

            // Clicking on a searchlist item will go to that page.
            this.$.searchedList.on('click', 'li', function (event) {
                var currentTarget = event.currentTarget;
                var url = $(currentTarget).find('a').attr('href');
                _this.moveToPage(url);
            });

            this.$.searchInput.on('keyup', function (event) {
                var inputText = _this.removeWhiteSpace(_this.$.searchInput.val()).toLowerCase();
                ...
                if (!inputText) {
                    _this.$.searchedList.html('');
                    return;
                }

                if (event.keyCode === KEY_CODE_ENTER) {
                    if (!_this.$.selected.length) {
                        _this.$.selected = _this.$.searchedList.find('li').first();
                    }
                    _this.moveToPage(_this.$.selected.find('a').attr('href'));
                    return;
                }

                _this.setList(inputText);
            });

            this.$.searchInput.on('keydown', function (event) {
                _this.$.selected.removeClass('highlight');

                switch (event.keyCode) {
                    case KEY_CODE_UP:
                        _this.$.selected = _this.$.selected.prev();
                        if (!_this.$.selected.length) {
                            _this.$.selected = _this.$.searchedList.find('li').last();
                        }
                        break;
                    case KEY_CODE_DOWN:
                        _this.$.selected = _this.$.selected.next();
                        if (!_this.$.selected.length) {
                            _this.$.selected = _this.$.searchedList.find('li').first();
                        }
                        break;
                ...
                _this.$.selected.addClass('highlight');
            });
        }
    }, {
        key: 'isMatched',
        value: function isMatched(itemText, inputText) {
            return this.removeWhiteSpace(itemText).toLowerCase().indexOf(inputText) > -1;
        }
    }, {
        key: 'moveToPage',
        value: function moveToPage(url) {
            if (url) {
                window.location = url;
            }
            this.clear();
        ...
        key: 'makeListItemHtml',
        value: function makeListItemHtml(item, inputText) {
        ...
        key: 'setList',
        value: function setList(inputText) {
            var _this2 = this;

            var html = '';

            this.$.anchorList.filter(function (idx, item) {
                return _this2.isMatched(item.text, inputText);
            }).each(function (idx, item) {
                html += _this2.makeListItemHtml(item, inputText);
            });
            this.$.searchedList.html(html);
        }
    }, {
        key: 'removeWhiteSpace',
        value: function removeWhiteSpace(value) {
